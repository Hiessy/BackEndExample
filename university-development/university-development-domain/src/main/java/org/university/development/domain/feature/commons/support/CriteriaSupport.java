package org.university.development.domain.feature.commons.support;
...
import org.university.development.domain.exception.InvalidCriteriaException;
...
public abstract class CriteriaSupport {
    ...
    public synchronized Order getOrder(Order order) {
        ...
        if (!foundCriteria || !foundCriteriaDirection) {
            if (!foundCriteria) {
                cause = new Cause(order.getProperty(), "Criteria's name not found");
